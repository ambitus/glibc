...
/* Register set for the vector registers.
   Technically we could combine this with the fpregset, since the
   floating point regs are the high bits of each of the first 16
   vector registers.  */
typedef struct
  {
    /* TODO: Is this all we need? Is there a vector state reg?  */
    vreg_t __ctx(vrs)[32];
  } vrregset_t;
...
typedef struct
  {
    ...
    /* TODO: Change the context-handling functions to be vr-aware.  */
    vrregset_t __ctx(vregs);
  } mcontext_t;
