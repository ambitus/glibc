		 ...
		 uint32_t flags, uint32_t flags2)
{
  ...
  uint64_t lx_ex_offset = (flags & STORAGE_REQ_RELEASE) ? 204 : 160;
  uint32_t flags2_temp = flags2;
  register uint32_t r0 asm ("r0") = length;
  register uint32_t r15 asm ("r15") = flags;

  /* z/OS TODO: Forcing r1 to be an input results in broken
     code being generated by gcc (something to do with how r1 is
     considered always live confusing ira and reload). Using it as an
     output nearly works, but it might generate incorrect code if gcc
     needs to refer to the incoming arg list after the asm.  */
  __asm__ __volatile__ ("lgr	%%r6, %%r1         \n\t"
			"lgr	%%r1, %6           \n\t"
			"ear	%%r7, %%a15        \n\t"
			"llgt	%%r14, 16          \n\t"
			"l	%%r14, 772(%%r14)  \n\t"
			...
			"lgr	%1, %%r1           \n\t"
			"lgr	%%r1, %%r6         \n\t"
			: "=r" (return_code), "=r" (ret_addr),
			  "+r" (flags2_temp), "+r" (r0)
			: "0" (r15), "r" (lx_ex_offset),
			  "r" (target_addr)
			: "r6", "r7", "r14", "cc");

  /* Return value is the address for allocation requests, and the
     return code for release requests.  */
		  ...
		  bool noexec, bool on_page_boundary)
{
  /* The storage obtain request is as follows:
     ...
     [,maybe CHECKZERO=YES]
     sp 131 is non auth, private low, fetch protected, pagable, owned
     by the job step task.
...
__storage_release (unsigned int storage_addr, unsigned int length)
{
  ...
  return (int) storage_request (length, storage_addr, flags, flags2);
}

/* Discards the data, replacing it with zeros.  Equivalent to
   STORAGE RELEASE followed by STORAGE OBTAIN.  Faster than
   MVCL for large areas.  The pages do not use resources. */
void
