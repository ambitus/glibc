...
#ifndef __OPENDIRWITHFD_H
#define __OPENDIRWITHFD_H
...
static DIR *
__opendir_with_fd (const char *name, int fd)
 {
  int32_t retval, reason_code;
  INTERNAL_SYSCALL_DECL (retcode);
  ...
  /* If fd is unknown open the directory to give those who wish to fcntl,
     or dup somthing to play with. */
  if (fd == -1)
    {
